		...
		<div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
			...
			<div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
				<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
					<h1 class="text-center text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">Create an account</h1>
					<form class="space-y-4 md:space-y-6" action="#" @submit.prevent="authStore.authenticate('register', formData)">
						<div>
							...
							<input
								...
								:class="[
									...
									authStore.errors.name ? 'border-red-600 focus:border-red-600 focus:ring-red-500 dark:border-red-500 dark:focus:ring-red-500' : 'border-gray-300',
								]"
								placeholder=""
								required />
							<p v-if="authStore.errors.name" class="mt-2 text-xs text-red-600 dark:text-red-500">
								<span v-for="(error, index) in authStore.errors.name" :key="index" class="block font-medium">
									{{ error }}
								</span>
							</p>
						</div>
						<div>
							...
							<p v-if="authStore.errors.email" class="mt-2 text-xs text-red-600 dark:text-red-500">
								<span v-for="(error, index) in authStore.errors.email" :key="index" class="block font-medium">
									{{ error }}
								</span>
							</p>
						</div>
						<div>
							...
							<p v-if="authStore.errors.password" class="mt-2 text-xs text-red-600 dark:text-red-500">
								<span v-for="(error, index) in authStore.errors.password" :key="index" class="block font-medium">
									{{ error }}
								</span>
							</p>
						</div>
